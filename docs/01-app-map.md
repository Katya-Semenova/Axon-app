...
| `/p/[id]` | Публичная презентация | все по приватной ссылке | Read-only показ презентации по токену шаринга, **вход не нужен** |
...
graph LR
  ...
  Public["Публичная презентация<br/>/p/[id]"]:::guest
  ...
  Entry -- "поделиться презентацией" --> Public
...
| `/p/[id]` | серверная загрузка по токену | — | слайды презентации | невалидный токен → «Презентация недоступна» |
...
> **Заметка:** в `/register` ссылки на `/terms` и `/privacy` сделаны обычным `<a href="/terms">`
> (не `<Link>`) — намеренно: Next не дописывает `basePath`, и они ведут на корень домена
> (`axon-app.ru/terms` — юр-страницы лендинга). Открываются в новой вкладке (`target=_blank`),
> чтобы не потерять заполненную форму. Работает корректно.
