...
- **Словарь токенов:** `--slide-bg` · `--slide-title` · `--slide-text` · `--slide-accent` ·
  ...
  задаёт `italic`** — журнальный «лид»; гротеск-темам курсив не идёт). **Фон сводки** во всех темах =
  `color-mix(muted 70%, bg)` — на ~30% мягче чистого `--slide-muted` (правило в компонентах, не токен) ·
