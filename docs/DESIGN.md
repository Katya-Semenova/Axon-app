...
- **Словарь токенов:** `--slide-bg` · `--slide-title` · `--slide-text` · `--slide-accent` ·
  ...
  `--slide-summary-style` (опц., 2026-07-04: начертание текста сводки; фолбэк `normal`. **Editorial
  задаёт `italic`** — журнальный «лид»; гротеск-темам курсив не идёт) ·
  `--slide-font-summary` (опц., 2026-07-04: гарнитура сводки; фолбэк = `--slide-font-display`.
  **Editorial задаёт Old Standard TT** — настоящий кириллический курсив «как в учебнике», вес 400,
  шрифт загружен в `layout.tsx`). **Фон сводки** во всех темах = `color-mix(muted 50%, bg)` —
  наполовину мягче чистого `--slide-muted` (правило в компонентах, не токен; было 70% → ещё светлее
  по просьбе 2026-07-04) ·
  ...
  фолбэк вне тем — крем→navy; **холст и drill-in задают фирменную GOLD→крем→NAVY через
  `CANVAS_HEAT_VARS` в `ui/tokens.ts`** — «холст как в слайдах», 2026-07-04) · `--slide-heat-mid` (опц., 2026-07-04: средний стоп шкалы; фолбэк —
  ровно середина from→to, поэтому двухтоновые темы рисуются без изменений) ·
  `--slide-heat-ink-mid`/`--slide-heat-ink-high` (опц.: цвет цифры в клетке на нейтральных (35–62%)
  и тёмных (>62%) клетках; фолбэк — противоположный конец шкалы с переключением на 50%, как раньше).
  **Editorial задаёт фирменную трёхстоповую шкалу GOLD `#B89548` → крем `#E9E4D5` → NAVY `#1B2840`,
  цифры navy / серый `#5C6478` / крем `#F5F2EA` — возвращён вид до 2026-07-02 (вариант Б).** ·
