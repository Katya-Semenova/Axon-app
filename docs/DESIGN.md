...
- Шрифты — гротеск (`'Helvetica Neue', Inter, Arial`), без serif; mono — JetBrains Mono
...
- Шрифты — гротеск (Inter), БЕЗ serif; mono — JetBrains Mono
...
### Web-dashboard → Raycast-проход (2026-06-27)
Тёмная (`web`) + светлая (`web-light`) под тумблером ☀/🌙. Прокачано в сторону Raycast: глубже почти-чёрная
нейтральная база, больше воздуха, скругление 12, **мягкая тень** карточки (`--slide-shadow`), приглушённый
вторичный текст; акцент — один, золото. Палитра серий не менялась (читается на тёмном).
- Тёмная: фон `#0A0B0F` · границы `#23272F` · muted `#14171E` · текст `#9AA6BA` · акцент `#D4A94E` ·
  радиус 12 · `--slide-block-pad: 26px 40px` · тень `0 18px 44px rgba(0,0,0,0.5)` · оси `#4F5A70`.
- Светлая: фон `#FAFBFD` · muted `#EEF1F6` · текст `#54607A` · акцент `#B8862E` · тень `0 14px 36px rgba(40,50,80,0.12)`.
- *Interactive-показ (отдать деку как живой веб-дашборд) — отдельная задача, тумблер пока скрыт.*
