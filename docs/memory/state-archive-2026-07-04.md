...
> Где мы сейчас + что осталось. Обновлено 2026-06-23.

## ⚠️ ВРЕМЕННЫЙ ПАРОЛЬ НА ПРИЛОЖЕНИЕ (с 2026-06-24) — НЕ ЗАБЫТЬ СНЯТЬ
✅ Поставлен на время показа арт-руководителю: **nginx Basic Auth на `/ai-studio`** (логин `axon`),
лендинг `/` открыт. Кода/git не касается → деплой пароль не снимает. **Это временно — СНЯТЬ после показа.**
...
## ◀ ТЕМЫ ДЕКИ (этот чат) — Swiss/Soft/Web-Raycast (2026-06-27)
Приоритет №1 «разная стилистика тем». Сделано+задеплоено: **Swiss** (красный, гротеск), **Soft** (пастель+лайм, тень),
**Web→Raycast** (фиолет, тёмный+светлый); summary-врез в публичной деке; шапка слайда (ритм/инсайт −13%/заголовочный
шрифт). Инфра: починен OOM деплоя (`NODE_OPTIONS=2048` в `apps/app/Dockerfile`).
**⚠️ ВЕРДИКТ: цвета мало → ЗАВТРА глобальная переделка** (бэклог §«Темы — глобальная переделка стилистики»):
(1) шрифтовые пары — всё разное, без Inter (Swiss=Saira Condensed 900/300, Soft=Nunito+Mulish, Web=Manrope+IBM Plex);
(2) форма графиков (скругление) — делаю Я в `ChartRenderer` (сосед сперва коммитит свой линт);
(3) «инфографика» (огромные числа) — отдельный крупный этап. Демо-режим стенда — **понедельник, п.0 бэклога**.
Журнал: `docs/decisions/sessions/2026-06-27-themes-swiss-soft-raycast.md`. Память: `project_presentation_themes_track`.
...
- **Шаг 2** — паспорта ВСЕХ экранов (`docs/screens/*.md`) сверены с кодом; удалены `signup.md`/`profile.md`.
- **Шаг 3** — `docs/screens/_global.md` (сквозные правила) + закрепление: правило №3 в `AGENTS.md` +
  PostToolUse-хук (`docs/rules/hooks-guardrails.md`).
- **Шаг 4** — `docs/test-cases.md` (6 BDD-сценариев). Прогон БЕЗ входа сделан вживую (HTTP к проду:
  admin→404, settings гостю→/login, битый токен→«недоступна»); под входом — прод-чек-лист (см. «Дальше»).
**Шаг 4 — ЗАКРЫТ (прод-чек-лист ПОД ВХОДОМ пройден вживую 2026-06-24):** сценарии 1/2/4/5/6 проверены на
проде (регистрация + флаги cookie HttpOnly/Secure/SameSite=Lax, неверный пароль→общая ошибка, /settings после
входа, файл→графики→персист, создание/отзыв публичной ссылки); 3 (IDOR) и 429 по rate-limit — на автотесты
финала (вручную не воспроизводим). По пути 4 UX-находки — в `backlog.md`. Подробности — `docs/test-cases.md`.
**Дальше:** (1) спеки холста/слайдов/показа сделаны, есть пометки «проверить» (Esc/клик-вне в оверлеях,
удаление/связи в drill-in, in-app Present); (2) **Шаг 5+** урока; (3) автотесты (e2e+интеграционные в гейте)
— финал курса (нужен стенд, Mac локально не тянет).
Журнал: `docs/decisions/sessions/2026-06-23-urok-07-steps-1-4.md`.
⚠️ Параллельно второй чат дорабатывает дизайн лендинга/прототайпа (его зона) — свою сессию закроет отдельно.
...
**Урок 6 (публичная упаковка) — ЗАВЕРШЁН (2026-06-22):**
