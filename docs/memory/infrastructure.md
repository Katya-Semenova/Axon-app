...
### Демо-аккаунт + кнопка «Попробовать демо» (Урок 7)

Гость по QR → экран входа → кнопка **«Попробовать демо»** (без регистрации). Кнопка дёргает
...
| `AI_IP_RATE_WINDOW` _(опц.)_ | окно лимита, сек | дефолт `3600` |

**После показа:** убрать `NEXT_PUBLIC_DEMO_ENABLED` (кнопка пропадёт) и вернуть Basic Auth (выше).
