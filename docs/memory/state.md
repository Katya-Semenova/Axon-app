...
Главный блокер деплоя СНЯТ + адаптив сделан. **Осталось одно: ДЕПЛОЙ руками пользователя → проверки на проде.**
Журнал сессии: `docs/decisions/sessions/2026-06-22-urok-06-topology-mobile.md`.
...
6. **✅ Шаг 5 — финальный адаптив — СДЕЛАН (2026-06-22, по коду).** Порог 767, зона бага ~760 закрыта
   (`body{overflow-x:clip}` + мобильные раскладки Hero/FAQ/Modes/Features/Prototype). Визуально НЕ проверен —
   на проде после деплоя. **◀ СЛЕДУЮЩАЯ ЗАДАЧА: ДЕПЛОЙ (п. 4) → проверки на проде (п. 5 + адаптив 375/767).**
7. Закрытие урока — `/save-session` (топология+адаптив зафиксированы 2026-06-22; финал — после деплоя).
