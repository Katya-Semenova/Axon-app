...
**Дальше:** (1) **прод-чек-лист тестов ПОД ВХОДОМ** (регистрация + флаги cookie HttpOnly/Secure/SameSite,
IDOR на 2 аккаунтах, загрузка→инсайты→сохранение, создание/отзыв ссылки); (2) спеки холста/слайдов/показа
сделаны, есть пометки «проверить» (Esc/клик-вне в оверлеях, удаление/связи в drill-in, in-app Present);
(3) **Шаг 5+** урока; (4) автотесты (e2e+интеграционные в гейте) — финал курса (нужен стенд, Mac локально не тянет).
