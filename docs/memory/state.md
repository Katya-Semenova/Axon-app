...
3. **ОСТАНОВИЛИСЬ НА: Шаг 5 — финальный адаптив (см. пункт 6 ниже). Это следующая задача.**
   ⚠️ **УРОК НА БУДУЩЕЕ (важно):** dev-сервер запускать РОВНО ОДИН раз через Bash `run_in_background`,
   и ОБЯЗАТЕЛЬНО гасить его через `TaskStop` по завершении. НЕ плодить `npm run dev` повторно,
   НЕ использовать `(npm ... &)` в обычном Bash (осиротит процесс). Иначе снова забьём процессы.
   Для замера адаптива лучше: один сервер → Playwright MCP на `http://localhost:3002` → измерить
   `document.documentElement.scrollWidth > clientWidth` на ширинах 1280/768/760/375 → найти узлы шире
   вьюпорта → чинить. Кандидат №1 на overflow: секция «Why Axon» — grid `566px 566px` (=1152+паддинги),
   фикс-ширины карточек 216px, абсолютное позиционирование; см. также THE SHIFT (500vh) и Hero-карточки.
