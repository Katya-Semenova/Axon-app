...
## Секреты и переменные окружения (Урок 3, Шаг 2 — снимок 2026-06-14)

На текущий момент у Axon **нет ни одного секрета**:

- Ноль обращений к `process.env`; нет API-роутов, БД, авторизации.
- «AI строит insight-карточки» работает на `lib/mockData.ts` (демо-данные на клиенте), **реального AI-ключа в проекте нет**.
- Наружу приложение ходит только за публичными Google Fonts (см. `globals.css`).
- Захардкоженных ключей не найдено. `.gitignore` уже прячет `.env*` и `.vercel`.

**Поэтому `.env.example` сознательно НЕ заводим** — нечего описывать, выдуманные переменные вводили бы в заблуждение.

**Когда вернуться:** как только подключим реальный AI вместо `mockData` (или auth/БД) — тогда:
1. завести `.env.example` с реальными именами переменных (без значений);
2. реальные значения положить в Vercel → Settings → Environment Variables **и** в бэкап Bitwarden (НЕ в iCloud, НЕ в git);
3. сначала зафиксировать новую логику в [spec.md](spec.md), потом код.
