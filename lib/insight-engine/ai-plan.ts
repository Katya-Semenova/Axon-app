...
const MULTI_METRIC_CHARTS = new Set<string>(["Stacked Bar", "Heatmap", "Radar"]);
...
export function buildExtractionMessages(input: AIExtractionInput): LLMMessage[] {
  const system = [
    ...
    "- metrics: массив имён ЧИСЛОВЫХ колонок (обычно одна; Scatter — две; Stacked Bar/Heatmap/Radar — несколько).",
    "",
    "Правила подбора графика: дата → Spline Area; мало категорий → Donut; несколько метрик по категории → Stacked Bar;",
    "две числовые без измерения → Scatter; много числовых → Heatmap; иначе Treemap/Lollipop.",
